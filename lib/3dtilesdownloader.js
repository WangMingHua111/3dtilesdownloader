...
async function download(url, searchParams, rootDir, breakpoint, parallel, count, root) {
    ...
    if (parallel) {
        ...
        for (let i = 0; i < chunk.length; i++) {
            await Promise.all(chunk[i].map(file => download(new URL(`${root}/${file}`, url.origin), searchParams, rootDir, breakpoint, parallel, count, root)))
        }
    }
    else
        for (let i = 0; i < subfiles.length; i++) {
            const file = subfiles[i]
            await download(new URL(`${root}/${file}`, url.origin), searchParams, rootDir, breakpoint, parallel, count, root)
        }
}
